package com.rzd.payment.service;

import com.rzd.common.dto.OrderDTO;
import com.rzd.common.dto.UserDTO;
import com.rzd.common.enums.OrderStatus;
import com.rzd.payment.client.OrderServiceClient;
import com.rzd.payment.client.UserServiceClient;
import com.rzd.payment.client.DocumentServiceClient;
import com.rzd.payment.model.dto.request.PaymentRequest;
import com.rzd.payment.model.dto.request.PaymentWebhookRequest;
import com.rzd.payment.model.dto.response.PaymentResponse;
import com.rzd.payment.model.entity.CompanyAccount;
import com.rzd.payment.model.entity.Payment;
import com.rzd.payment.model.entity.Payment.PaymentStatus;
import com.rzd.payment.repository.CompanyAccountRepository;
import com.rzd.payment.repository.PaymentRepository;
...
public class PaymentService {
    ...
    // Feign Clients вместо прямых репозиториев
    ...
    public Payment createCorporatePayment(PaymentRequest request, String userEmail) {
        ...
        // Проверяем статус заказа через Order Service
        OrderDTO order = orderServiceClient.getOrderDTO(request.getOrderId());
        if (order == null) {
            throw new RuntimeException("Заказ не найден");
        }

        if (order.getStatus().equals(OrderStatus.оплачен.name())) {
            throw new RuntimeException("Заказ уже оплачен");
        }

        // Работа со счетом (это логика payment-service, оставляем)
        ...
        // Обновляем статус заказа через Order Service
        if (savedPayment.getOrderId() != null) {
            try {
                orderServiceClient.updateOrderStatus(savedPayment.getOrderId(), OrderStatus.оплачен);
                log.info("Статус заказа {} обновлен на 'оплачен' через Order Service", savedPayment.getOrderId());
            } catch (Exception e) {
            ...
            switch (request.getStatus()) {
                case "succeeded":
                    ...
                    if (payment.getOrderId() != null) {
                        try {
                            orderServiceClient.updateOrderStatus(payment.getOrderId(), OrderStatus.оплачен);
                            log.info("Статус заказа {} обновлен на оплачен", payment.getOrderId());
                        } catch (Exception e) {
                            log.error("Ошибка при обновлении статуса заказа: {}", e.getMessage());
                        }
                    }
    ...
    public boolean isOrderPaid(UUID orderId) {
        // Проверяем через Order Service
        try {
            OrderDTO order = orderServiceClient.getOrderDTO(orderId);
            return order != null && order.getStatus().equals(OrderStatus.оплачен.name());
        } catch (Exception e) {
            log.error("Ошибка при проверке статуса заказа: {}", e.getMessage());
            return false;
        }
    }
    ...
    public byte[] generateInvoicePdf(UUID paymentId) {
        ...
        try {
            // Вызываем Document Service для генерации PDF
            return documentServiceClient.generateInvoice(paymentId);
        } catch (Exception e) {
            log.error("Ошибка при генерации PDF для платежа {}", paymentId, e);
            throw new RuntimeException("Не удалось создать PDF документ: " + e.getMessage());
        }
    }
}